...
int main() {
    ...
    repi (i, 1, n) {
        repi (j, i + 1, n) {
            if (abs(XY.at(i - 1).first - XY.at(j - 1).first) < d) {
                ts.add_clause(i - 1, false, j - 1, false);
            }
            if (abs(XY.at(i - 1).first - XY.at(j - 1).second) < d) {
                ts.add_clause(i - 1, false, j - 1, true);
            }
            if (abs(XY.at(i - 1).second - XY.at(j - 1).first) < d) {
                ts.add_clause(i - 1, true, j - 1, false);
            }
            if (abs(XY.at(i - 1).second - XY.at(j - 1).second) < d) {
                ts.add_clause(i - 1, true, j - 1, true);
            }
        }
    }
    if (ts.satisfiable()) {
        ...
        repi (i, 1, n) {
            if (ans.at(i - 1)) {
                cout << XY.at(i - 1).first << endl;
            }
            else {
                cout << XY.at(i - 1).second << endl;
            }
        }
    }
    else {
